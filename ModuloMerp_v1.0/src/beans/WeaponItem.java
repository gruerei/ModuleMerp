...
public class WeaponItem extends Item implements Cloneable {
	...
	/**
	 * TODO Los casos donde un arma puede usarse de dos formas: Por defecto se
	 * usará lo que viene en el .properties en su categoria Bastard Sword and
	 * Double Axe : Por defecto (EDGED - 1 HAND) DAGGER, AXE : Por defecto
	 * (EDGED - 1 HAND)
	 * 
	...
	private int category;// Filo, Contundente, 2 Manos, Arrojadiza, Proyectiles
							// ,Asta
	private int quality;// Normal, High, Magical, Mithril, Holy, Slaying
	private Botch botch;
	private Critical mainCritical;
	// Si se obtiene un critico mayor de B, puede haber derecho a un critico
	// secundario
	...
	public WeaponItem(String type, int category, int quality, Critical main_critical, Critical second_critical,
			Botch botch, float range, float weight, int typeMod1, Integer[] typeMod1Applied, int typeMod2,
			Integer[] typeMod2Applied, int specialMod1, String[] specialMod1Applied, int specialMod2,
			String[] specialMod2Applied, int reloadAssaults, int malusNoReload, Price price_obj, boolean usedTwoHanded,
			Botch twoHandedBotch, Critical twoHandedCritical, int twoHandedMod, int[] skillMods) {
	...
	public WeaponItem(String type, int category, int quality, Critical main_critical, Critical second_critical,
			Botch botch, float range, float weight, int specialMod1, String[] specialMod1Applied, int specialMod2,
			String[] specialMod2Applied, int reloadAssaults, int malusNoReload, Price price_obj, boolean usedTwoHanded,
			Botch twoHandedBotch, Critical twoHandedCritical, int twoHandedMod, int[] skillMods) {
	...
	public String toString() {
		...
		StringBuffer sb = new StringBuffer("\n\n").append(name == null ? "" : name + " - ").append(type).append(" - ")
				.append(Tables.getSkillCategoryTable()[category][0]).append(outputBOChain).append("\nCritico 1º:")
				.append(mainCritical.toString()).append("\t2º:")
				.append(secondCritical != null ? secondCritical.toString() : "").append("\nPifia: ")
				.append(botch.getMin()).append(" - ").append(botch.getMax()).append(" Crit. Taken: ")
				.append((botch.getCriticalTaken() == null) ? "NO" : botch.getCriticalTaken().getCriticalMaxGravity())
				.append("\nRange: ").append(range).append(" Weight: ").append(weight).append(" kg.")
				.append("\nType Mod1: ").append(typeMod1).append(" Applied To: ")
				.append(Utils.printTable(typeMod1AppliedTo)).append("\nType Mod2: ").append(typeMod2)
				.append(" Applied To: ").append(Utils.printTable(typeMod2AppliedTo)).append("\nSpecial Mod1: ")
				.append(specialMod1).append(" Applied To: ").append(Utils.printTable(specialMod1AppliedTo))
				.append("\nSpecial Mod2: ").append(specialMod2).append(" Applied To: ")
				.append(Utils.printTable(specialMod2AppliedTo)).append("\nReaload Assaults: ").append(reloadAssaults)
				.append(" Malus No Reload: ").append(malusNotReload).append("\nPrice: ").append(super.price.toString());
		;
		if (this.usedTwoHanded) {
			sb.append("\n2 HANDED PROPERTIES").append("\nCritico 2H: ")
					.append(twoHandedCritical == null ? "NO" : twoHandedCritical.toString()).append("\nPifia 2H: ")
					.append(twoHandedBotch == null ? "NO" : twoHandedBotch.getMin() + " - " + twoHandedBotch.getMax())
					.append(" Crit. Taken: ")
					.append((twoHandedBotch.getCriticalTaken() == null) ? "NO"
							: twoHandedBotch.getCriticalTaken().getCriticalMaxGravity())
	...
	public WeaponItem clone() {
		WeaponItem clon = new WeaponItem(this.type, this.category, this.quality, this.mainCritical, this.secondCritical,
				...
				this.typeMod2AppliedTo, 0, new String[10], 0, new String[10], this.reloadAssaults, this.malusNotReload,
				this.price, this.usedTwoHanded, this.twoHandedBotch, this.twoHandedCritical, this.twoHandedMod,
				new int[Skill.SKILLS_TOTAL_NUMBER]);
		return clon;
	}
	...
	/**
	 * This Method checks if The Weapon Buff/Debuff against a specific type of
	 * Armour
	 */
	public static int calculWeaponTypeBonus(WeaponItem weapon, ArmourItem enemyArmour) {
		int bonus = 0;
		int arCat = 0;

		if(enemyArmour == null ){
			arCat = ArmourItem.getMovementSkillByArmour(null);
		}else{
			arCat = ArmourItem.getMovementSkillByArmour(enemyArmour.getType());
		}
		
		
		if (weapon.getTypeMod1() != 0 && weapon.getTypeMod1AppliedTo() != null) {
			// Si no hay restriccion
			// Mira si el Tipo de Armadura tiene alguna bono respecto al
			// Tipo de Arma usada
			boolean apply = Utils.findValueInArray(arCat, weapon.getTypeMod1AppliedTo());

			// En caso afirmativo, aplica el bono
			if (apply) {
				System.out.println(weapon.getType() + " tiene bono de " + weapon.getTypeMod1() + " respecto a " + enemyArmour.getType());
				bonus = bonus + weapon.getTypeMod1();
			}
		}

		// Idem para el Tipo2
		if (weapon.getTypeMod2() != 0 && weapon.getTypeMod2AppliedTo() != null) {
			boolean apply2 = Utils.findValueInArray(arCat, weapon.getTypeMod2AppliedTo());
			if (apply2) {
				System.out.println(weapon.getType() + " tiene bono de " + weapon.getTypeMod2() + " respecto a " + enemyArmour.getType());
				bonus = bonus + weapon.getTypeMod2();
			}
		}
